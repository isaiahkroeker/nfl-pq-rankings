...
def get_nfl_data():
    ...
    # Smart Mapping to prevent KeyErrors
    mapping = {
...
<head>
    ...
    <style>
        ...
        .card {{ background: var(--card); padding: 15px; border-radius: 12px; margin: 10px; display: flex; align-items: center; border-left: 5px solid; }}
        .logo {{ width: 45px; height: 45px; margin-right: 15px; }}
        .pq-val {{ font-weight: bold; color: #38bdf8; font-size: 1.2em; }}
        .badge {{ font-size: 0.7em; padding: 4px 8px; border-radius: 6px; margin-top: 5px; display: inline-block; }}
    </style>
</head>
<body>
    <div class="header"><h1>🏈 NFL PQ DASHBOARD</h1><p>Sync: {time_str}</p></div>
    {"".join([f'''
    <div class="card" style="border-left-color: {r['color']}">
        ...
        <div style="flex-grow: 1;">
            ...
            <div class="badge" style="background: {r['s_color']}">{r['status']}</div>
        </div>
