...
def get_nfl_data():
    ...
    # Calculate records including playoff games
    ...
    # 5. Build Playoff Dashboard
    dashboard = []
    for t in team_list:
        tg = played[(played['home_team'] == t) | (played['away_team'] == t)]
        
        # PQ Math: Values Playoff wins much higher
        ...
        # Status Logic: If they didn't play in the most recent playoff week, they are likely out
    ...
    return pd.DataFrame(dashboard).sort_values('pq', ascending=False).merge(teams_meta[['team_abbr', 'team_logo_espn', 'team_color']], left_on='team', right_on='team_abbr', how='left')
...
<head>
    ...
    <style>
        ...
        .card {{ background: var(--card); border-radius: 12px; padding: 15px; margin-bottom: 10px; display: flex; align-items: center; border-left: 6px solid; cursor: pointer; transition: 0.2s; }}
        ...
        .p-img {{ width: 50px; height: 50px; border-radius: 50%; margin-right: 15px; background: #334155; }}
        .close-btn {{ width: 100%; background: #ef4444; border: none; color: white; padding: 15px; border-radius: 10px; margin-top: 20px; font-weight: bold; font-size: 1.1em; cursor: pointer; }}
    </style>
</head>
<body>
    <div class="header">
        <h1>🏆 PLAYOFF PQ</h1>
        <p><b>Updated: {time_str}</b><br>Tap team for Playoff Roster</p>
    </div>
    {"".join([f'''
    <div class="card" style="border-left-color: {r['team_color']}" onclick="openModal('{r['team']}', `{r['players']}`)">
        <img src="{r['team_logo_espn']}" class="logo">
        <div>
            <b>{r['team']}</b><br>
            <small>{r['record']} • <span style="color:{r['s_color']}">{r['status']}</span></small>
        </div>
    ...
    <div id="modal" onclick="closeModal()">
        <div class="modal-content" onclick="event.stopPropagation()">
            <h2 id="m-title" style="margin-top:0; color:var(--accent);"></h2>
            <div id="m-body"></div>
