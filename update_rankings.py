...
def get_nfl_data():
    ...
    for t in team_list:
        ...
        status = "ALIVE" if not played[((played['home_team'] == t) | (played['away_team'] == t)) & (played['week'] == max_week)].empty else "ELIMINATED"
...
<head>
    ...
    <style>
        ...
        .modal-content {{ background: #1e293b; width: 85%; max-width: 350px; border-radius: 20px; padding: 25px; border: 1px solid #334155; }}
        .p-row {{ display: flex; align-items: center; padding: 10px 0; border-bottom: 1px solid #334155; }}
        .p-img {{ width: 45px; height: 45px; border-radius: 50%; margin-right: 12px; }}
...
<body>
    <div style="text-align:center;"><h1>🏆 PLAYOFF PQ</h1><p><b>{time_str}</b> | Tap for rosters</p></div>
    {"".join([f'''
